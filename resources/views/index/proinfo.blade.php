  ...
  <body>
    ...
    <div class="maincont">
     ...
     <div id="sliderA" class="slider">
      <img src="{{env('UPLOAD_URL')}}{{$post->goods_img}}" />
     </div><!--sliderA/-->
     <table class="jia-len">
      <tr>
       <th><strong class="orange" >¥{{$post->goods_price}}</strong></th>
       <td>
      ...
      <tr>
       <td>
        <strong>{{$post->goods_name}}</strong>
        <p class="hui">{{$post->goods_desc}}</p>
       </td>
       <td align="right">
     ...
     <div class="proinfoList">
      <img src="{{env('UPLOAD_URL')}}{{$post->goods_img}}" width="636" height="822" />
     </div><!--proinfoList/-->
     <div class="proinfoList">
      暂无信息....
     </div><!--proinfoList/-->
     ...
     <table class="jrgwc">
      <tr>
       ...
       <td><a class="addCart" href="{{url('/car/'.$post->goods_id)}}">加入购物车</a></td>
      </tr>
     </table>
    </div><!--maincont-->
...
$(document).on("click",".addCart",function(){
    var goods_id="{{$post->goods_id}}}";
    var goods_price="{{$post->goods_price}}}";
    $.ajax({
        headers:{
